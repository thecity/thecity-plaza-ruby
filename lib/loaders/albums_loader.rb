...
class AlbumsLoader < BaseLoader
  ...
  def initialize(subdomain, num_per_page = 10, cacher = nil, group_nickname = nil) 
    ...
    @class_key = "albums_#{num_per_page}_#{group_nickname}" 
    
    # The URL to load the albums from. 
    @url = "http://#{subdomain}.onthecity.org/plaza#{nickname}/albums?format=json&per_page=#{num_per_page}"
